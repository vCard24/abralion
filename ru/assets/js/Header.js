...
class Header {
  ...
  ensureCompareNavLink() {
    ...
    const href = headerCompare?.getAttribute('href') || `${base}sravnenie.html`;
  ...
  setDropdownArrowsInteractive(enabled) {
    ...
    this.headerNav.querySelectorAll('.dropdown-arrow').forEach((arrow) => {
      if (enabled && this.mobileMq.matches) {
        ...
        arrow.setAttribute('aria-label', typeof t === 'function' ? t('menu.submenuToggle') : 'menu.submenuToggle');
        return;
      }
  ...
  performSearch() {
    ...
    if (query) {
      this.setMobileNavOpen(false);
      window.location.href = `${base}produkty.html?search=${encodeURIComponent(query)}`;
    }
  }
  ...
  highlightActivePage() {
    ...
    this.navLinks.forEach((link) => {
      ...
      if (inProductPage && linkPage === 'produkty.html') isActive = true;
