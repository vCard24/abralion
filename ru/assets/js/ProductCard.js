...
class ProductCard {
  ...
  static categoryTags(product) {
    ...
    if (product.categoryId === 'kirici-delici') {
      tags.push(typeof t === 'function' ? t('productCard.industrial') : 'productCard.industrial');
    }
    ...
    card.innerHTML = `
      ...
      <div class="product-card-content flex flex-1 flex-col gap-3 p-5">
        ...
        <div class="product-card-footer mt-auto flex items-end justify-between gap-3 border-t border-steel-gray/20 pt-4">
          ...
          <a href="${url}" class="product-card-detail flex h-10 w-10 shrink-0 items-center justify-center rounded text-abrasive-red transition-colors hover:bg-abrasive-red/10" aria-label="${typeof t === 'function' ? t('productCard.detailsAria', { name }) : name}">${ProductCard.detailIconSvg()}</a>
        </div>
      </div>
    `;
    return card;
  ...
  createCardElement() {
    ...
    const variantId = firstVar ? firstVar.id || firstVar.urun_kodu : p.id;
    const kartSrc = ProductCard.kartImageSrc(base, p.slug, p);
    const url = productUrl(p.slug);
      ...
      : `<div class="product-card-actions flex items-center justify-between pt-6 border-t border-steel-gray/10">
          ...
          <div class="flex items-center gap-3 shrink-0">
            <button type="button" class="btn-icon-compare flex h-10 w-10 items-center justify-center rounded text-steel-gray transition-colors hover:bg-surface-container-high hover:text-abrasive-red"
              title="${typeof t === 'function' ? t('productCard.compareFirst') : 'productCard.compareFirst'}"
              data-product-id="${p.id}" data-variant-id="${variantId}" aria-label="${typeof t === 'function' ? t('productCard.compare') : 'productCard.compare'}">
            ...
            <a href="${url}" class="product-card-detail flex items-center gap-2 font-label-caps group/btn transition-all text-on-surface hover:text-abrasive-red"
              data-product-id="${p.id}" aria-label="${typeof t === 'function' ? t('productCard.detailsAria', { name }) : name}">
              ${typeof t === 'function' ? t('productCard.details') : 'productCard.details'}
