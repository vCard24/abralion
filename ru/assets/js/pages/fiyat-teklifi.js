(function () {
  ...
  function bindStaticHandlers() {
    ...
    document.getElementById('quote-new-request')?.addEventListener('click', () => {
      ...
      window.location.href = url.pathname.split('/').pop() || 'zapros-tseny.html';
    });
  ...
  async function loadCatalog() {
    if (window.ABRALION_CATALOG?.products?.length) {
      ...
      throw new Error(t('quote.catalog.notFound'));
    }

    if (!products.length) {
      throw new Error(t('quote.catalog.emptyProducts'));
    }
    if (!categories.length) {
      throw new Error(t('quote.catalog.emptyCategories'));
    }
  ...
  function applyInitialRows() {
    ...
    if (fromCompare && keys.length && !initialRows.length) {
      showToast(t('quote.compareImport.failed'), 'error');
    }
  ...
  function showPrefillBanner(count) {
    ...
    if (text) {
      text.textContent =
        count === 1
          ? t('quote.prefillBanner.one')
          : t('quote.prefillBanner.many', { count });
    }
    banner.hidden = false;
  ...
  function slotMediaHtml(product) {
    if (!product) {
      return `<div class="quote-product-slot__placeholder" aria-hidden="true">
        ...
        <span class="quote-product-slot__placeholder-text">${t('quote.form.selectProduct')}</span>
      </div>`;
    }
  ...
  function addProductRow(prefill = {}) {
    ...
    row.innerHTML = `
      <div class="quote-product-slot__head">
        <p class="quote-product-slot__label">${t('quote.form.productLabel', { index })}</p>
      </div>
      <div class="quote-product-slot__layout">
        ...
        <div class="quote-product-slot__selects">
          <div class="quote-field">
            <label>${t('quote.form.category')}</label>
            <select class="quote-select quote-field-category" aria-label="${t('quote.form.categoryAria', { index })}">
              <option value="">${t('quote.form.categorySelect')}</option>
              ${categories
                .slice()
          ...
          <div class="quote-field">
            <label>${t('quote.form.product')}</label>
            <select class="quote-select quote-field-product" disabled aria-label="${t('quote.form.productAria', { index })}">
              <option value="">${t('quote.form.productSelectFirst')}</option>
            </select>
          </div>
          <div class="quote-field">
            <label>${t('quote.form.variant')}</label>
            <select class="quote-select quote-field-variant" disabled aria-label="${t('quote.form.variantAria', { index })}">
              <option value="">${t('quote.form.variantSelectFirst')}</option>
            </select>
          </div>
          <div class="quote-field">
            <label>Miktar <span class="quote-optional-tag">opsiyonel</span></label>
            <input type="text" class="quote-input quote-field-qty" placeholder="${t('quote.form.qtyPlaceholder')}" aria-label="${t('quote.form.qtyAria', { index })}">
          </div>
  ...
  function onCategoryChange(row, productId = '', variantId = '') {
    ...
    productSel.innerHTML = `<option value="">${t('quote.form.productSelect')}</option>`;
    variantSel.innerHTML = `<option value="">${t('quote.form.variantSelectFirst')}</option>`;
  ...
  function onProductChange(row, variantId = '') {
    const productId = row.querySelector('.quote-field-product').value;
    const variantSel = row.querySelector('.quote-field-variant');
    variantSel.innerHTML = `<option value="">${t('quote.form.variantSelect')}</option>`;
    variantSel.classList.remove('error');
  ...
  function validateForm(data, { requireContact = true } = {}) {
    ...
    if (!data.products.length) {
      showRowError(t('quote.validation.productsRequired'));
    ...
    const required = [
      ['quote-name', data.name, t('quote.validation.nameRequired')],
      ['quote-phone', data.phone, t('quote.validation.phoneRequired')],
      ['quote-email', data.email, t('quote.validation.emailRequired')],
      ['quote-country', data.country, t('quote.validation.countryRequired')],
      ['quote-city', data.city, t('quote.validation.cityRequired')],
    ];
    ...
    if (data.email && !/^[^\s@]+@[^\s@]+\.[^\s@]+$/.test(data.email)) {
      showFieldError('quote-email', t('quote.validation.emailInvalid'));
      ok = false;
    }

    if (!data.kvkk) {
      showFieldError('quote-kvkk', t('quote.validation.kvkkRequired'));
      ok = false;
    }
  ...
  function buildPrintHtml(data) {
    ...
    const cards = (data.products || [])
      .map((row, i) => {
        ...
        const imgCell = imgSrc
          ...
          : `<div style="font-size:11px;color:#9ca3af;text-align:center">${t('quote.document.noImage')}</div>`;
        ...
        return `<article style="border:1px solid #e5e7eb;border-radius:6px;overflow:hidden;background:#fff">
          <div style="position:relative;padding:8px 12px 0">
            <span style="display:inline-block;padding:2px 8px;border-radius:999px;background:#E2231A;color:#fff;font-size:9px;font-weight:700;letter-spacing:.06em;text-transform:uppercase">${t('quote.document.productN', { n: i + 1 })}</span>
          </div>
    ...
    const details = [
      data.application && `${t('quote.document.application')}: ${escapeHtml(data.application)}`,
      data.volume && `${t('quote.document.qty')}: ${escapeHtml(data.volume)}`,
      data.delivery && `${t('quote.document.delivery')}: ${escapeHtml(data.delivery)}`,
      data.urgency && `${t('quote.document.urgency')}: ${escapeHtml(urgencyLabel(data.urgency))}`,
    ...
    return (
      `<!DOCTYPE html><html lang="${typeof getLocale === 'function' ? getLocale() : 'ru'}"><head><meta charset="UTF-8"><title>${t('quote.mailHtmlTitle')}</title>
<style>
@page { margin: 14mm; }
...
${logoHtml}
<h1>${t('quote.document.title')}</h1>
<p class="meta">${escapeHtml(dateStr)}${data.reference ? ` · Ref: ${escapeHtml(data.reference)}` : ''}</p>
<h2>${t('quote.document.selectedProducts')}</h2>
<div class="products">${cards || `<p>${t('quote.document.noProducts')}</p>`}</div>
<h2>${t('quote.document.contact')}</h2>
<p class="contact"><strong>${escapeHtml(data.name || '—')}</strong><br>
${data.phone ? `${t('quote.document.phone')}: ${escapeHtml(data.phone)}<br>` : ''}
${data.email ? `${t('quote.document.email')}: ${escapeHtml(data.email)}<br>` : ''}
${data.company ? `${t('quote.document.company')}: ${escapeHtml(data.company)}<br>` : ''}
${escapeHtml(data.country || '—')} / ${escapeHtml(data.city || '—')}</p>
<h2>${t('quote.document.requestDetails')}</h2>
<p class="contact">${details || '—'}</p>
<p class="footer">EKS-PLAST LLC · info@abralion.com · www.abralion.com · 8 (495) 142-42-67<br>
${t('quote.document.footnote')}</p>
<script>window.onload=function(){window.focus();window.print();};</scr` +
  ...
  function handleSubmit(e) {
    ...
    if (!validateForm(data, { requireContact: true })) {
      showToast(t('quote.validation.requiredFields'), 'error');
      return;
    }
    ...
    sendFormMail(mailPayload)
      ...
      .catch((err) => {
        showToast(err.message || t('quote.toast.mailError'), 'error');
      })
      .finally(() => {
  ...
  function urgencyLabel(value) {
    const map = {
      normal: t('quote.delivery.normal'),
      urgent: t('quote.delivery.urgent'),
      stock: t('quote.delivery.stock'),
    };
  ...
  function buildPdfProductCards(data) {
    ...
    data.products.forEach((row) => {
      ...
      const imgHtml = imgSrc
        ...
        : `<div class="quote-pdf-card__img-placeholder">${t('quote.document.noImage')}</div>`;
      const specHtml = specLines
        .map((line) => pdfMetaItem(line.label || line[0] || '', line.value || line[1] || ''))
        .join('');
      cards.push(`<article class="quote-pdf-card">
        <div class="quote-pdf-card__badge">${t('quote.document.productN', { n: cards.length + 1 })}</div>
  ...
  function buildPdfSheet(data) {
    ...
    const productsHtml =
      buildPdfProductCards(data) || `<p class="quote-pdf-empty">${t('quote.document.noProducts')}</p>`;

    const contactHtml =
      pdfField(t('quote.document.fullName'), data.name) +
      pdfField(t('quote.document.phone'), data.phone) +
      pdfField(t('quote.document.email'), data.email) +
      pdfField(t('quote.document.company'), data.company) +
      pdfField(t('quote.document.country'), data.country) +
      pdfField(t('quote.document.city'), data.city);

    const detailParts = [
      data.application ? pdfField(t('quote.document.applicationArea'), data.application, true) : '',
      data.volume ? pdfField(t('quote.document.estimatedVolume'), data.volume) : '',
      data.delivery ? pdfField(t('quote.document.deliveryRegion'), data.delivery) : '',
      data.urgency ? pdfField(t('quote.document.deliveryUrgency'), urgencyLabel(data.urgency)) : '',
      data.message ? pdfField(t('quote.document.message'), data.message, true) : '',
    ].filter(Boolean);

    const detailsBlock = detailParts.length
      ? pdfBlock(t('quote.document.requestDetails'), `<div class="quote-pdf-fields">${detailParts.join('')}</div>`)
      : '';
    ...
    sheet.innerHTML = `<div class="quote-pdf-doc">
        ...
        <table class="quote-pdf-doc__header-table" role="presentation" cellspacing="0" cellpadding="0">
          <tr>
            ...
            <td class="quote-pdf-doc__header-main">
              <h1>${t('quote.document.title')}</h1>
              <p class="quote-pdf-doc__date">${t('quote.document.dateLabel')}: ${escapeHtml(dateStr)}${data.reference ? ` · Ref: ${escapeHtml(data.reference)}` : ''}</p>
            </td>
          </tr>
        </table>
      </header>
      ${pdfBlock(t('quote.document.selectedProducts'), `<div class="quote-pdf-products">${productsHtml}</div>`)}
      ${pdfBlock(t('quote.document.contactInfo'), `<div class="quote-pdf-fields">${contactHtml}</div>`)}
      ${detailsBlock}
      <footer class="quote-pdf-doc__footer">
        ...
        <p class="quote-pdf-doc__footnote">${t('quote.document.footnote')}</p>
      </footer>
    </div>`;
  ...
  function runQuoteDocumentPipeline({ mode, btnId, onSuccess, onFallback }) {
    ...
    if (!validateForm(data, { requireContact: false })) {
      showToast(t('quote.validation.minProducts'), 'error');
      return Promise.resolve();
    }
    ...
    if (mode === 'pdf' && (!h2c || !JsPDF)) {
      showToast(t('quote.toast.pdfModuleError'), 'error');
      return Promise.resolve();
    }
    ...
    return resolveAllProductImages(data.products)
      ...
      .then(() => {
        ...
        return captureSheetToCanvas(ctx.sheet, h2c).then((canvas) => {
          ...
          pdf.save(
            `${t('quote.file.prefix')}-${data.reference || t('quote.file.summary')}.pdf`
          );
          onSuccess?.();
        });
      })
  ...
  function downloadPdf() {
    runQuoteDocumentPipeline({
      mode: 'pdf',
      btnId: 'quote-pdf-btn',
      onSuccess: () => showToast(t('quote.toast.pdfDownloaded'), 'success'),
      onFallback: (data) => {
        if (printViaIframe(buildPrintHtml(data))) {
          showToast(t('quote.toast.pdfFallback'));
        } else {
          downloadHtmlFile(buildPrintHtml(data), data.reference);
          showToast(t('quote.toast.pdfHtmlFallback'), 'error');
        }
      },
    });
  }
  ...
  function printQuote() {
    runQuoteDocumentPipeline({
      ...
      onSuccess: () => showToast(t('quote.toast.printOpened'), 'success'),
      onFallback: (data) => {
        downloadHtmlFile(buildPrintHtml(data), data.reference);
        showToast(t('quote.toast.printFallback'), 'error');
      },
    });
  }
  ...
  function printViaIframe(html) {
    try {
      ...
      if (!frame) {
        ...
        frame.title = t('quote.print.frameTitle');
  ...
  function downloadHtmlFile(html, reference) {
    ...
    a.href = url;
    a.download = `${t('quote.file.prefix')}-${reference || t('quote.file.summary')}.html`;
